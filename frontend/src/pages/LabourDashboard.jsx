      ...
      <section className="space-y-3">
        ...
        <div className="-mx-4 flex gap-2 overflow-x-auto px-4 pb-1 scrollbar-none">
          {[
            ...
            const active = selectedWorkTypes.includes(key)
            return (
              <button key={key} type="button" onClick={() => toggleWorkType(key)}
                className={`shrink-0 rounded-full border px-4 py-2 text-xs font-semibold transition ${active ? 'border-amber-400 bg-amber-400 text-slate-900' : 'border-slate-200 bg-white text-slate-600'}`}>
                {label}
              </button>
            )
          })}
